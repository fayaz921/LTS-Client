...
export const ProfilePage = () => {
  ...
  // Reset form data when profile is loaded or changed, but only if not currently editing
  useEffect(() => {
          ...
          <div className="position-relative flex-shrink-0" style={{ cursor: 'pointer' }}
            ...
            {previewUrl || profile?.profilePictureUrl ? (
              ...
              <div className="rounded-circle d-flex align-items-center justify-content-center fw-bold"
                style={{ width: 80, height: 80, background: '#c89b2a', fontSize: '1.6rem', border: '3px solid rgba(255,255,255,0.15)' }}>
                {initials}
              </div>
            )}
            ...
            <div className="card border-0 shadow-sm h-100">
              <div className="card-body">
                <div className="d-flex justify-content-between align-items-center mb-2">
                  <span className="small fw-bold text-uppercase text-muted" style={{ fontSize: '0.65rem', letterSpacing: '0.08em' }}>
                    {stat.label}
                  </span>
              ...
              {!isEditing ? (
                <div>
                  {[
                    ...
                    <div key={i} className="d-flex align-items-center gap-3 py-2 border-bottom">
                      ...
                      <div>
                        <div className="text-muted text-uppercase" style={{ fontSize: '0.62rem', letterSpacing: '0.05em' }}>{item.label}</div>
                        <div className="fw-medium" style={{ fontSize: '0.85rem' }}>{item.value || '—'}</div>
                      </div>
                    </div>
                  ))}
                </div>
                ...
                <div>
                  {([
                    ...
                    { label: 'Location', key: 'location', type: 'text', placeholder: 'City, Pakistan' },
                  ] as { label: string; key: keyof UpdateProfileCommand; type: string; placeholder: string }[]).map((field, i) => (
                  ...
                  <div className="mb-3">
                    ...
                    <select className="form-select" style={{ fontSize: '0.85rem', borderRadius: 8 }}
                      value={formData.department}
                      onChange={e => setFormData({ ...formData, department: e.target.value })}>
                      <option value="">Select department</option>
                      {['Administration', 'Legal', 'Finance', 'Operations', 'IT'].map(d => (
            ...
            <div className="card-body">
              ...
              <div className="rounded-3 p-3 mb-3" style={{ background: '#f8fafc', border: '1px solid #e2e8f0' }}>
                ...
                {[
                  ...
                  <div key={item.step} className="d-flex align-items-center gap-2 mb-2" style={{ fontSize: '0.78rem' }}>
